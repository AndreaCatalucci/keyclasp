...
This is the implemented design. Objective-C keeps the helper independent of the Swift compiler/runtime version mismatch encountered by the disposable Swift prototype. The packaged bundle is arm64-only, ad-hoc signed as `dev.keyclasp.biometric` with hardened runtime and no entitlements, and invoked through its inner executable.
...
- Before launch, Keyclasp validates the executable and bundle layout, ownership, mode bits, write-granting ACLs, manifest hashes, strict signature, hardened-runtime flag, absence of entitlements, identifier, designated requirement, and architecture. It invokes the executable directly with a fixed minimal environment. It does not use `open`, AppleScript, a daemon, or a shell.
- These checks fail before the helper runs. They detect a damaged or accidentally replaced package but do not constrain arbitrary code already running as the same user or prevent a modified Keyclasp caller from bypassing the helper.
...
The helper changed the packaged biometric path, making rc4 historical. rc5 exposed the invalid `LSBackgroundOnly` lifecycle during physical verification and is also historical. rc6 used the earlier reviewed accessory-app lifecycle and passed its exact-artifact matrix plus physical macOS Touch ID verification. Those receipts remain historical and do not qualify the current hardened helper. Publication remains a separate protected checkpoint.
