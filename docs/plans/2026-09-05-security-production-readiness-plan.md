...
- PR validation exposed a Linux portability defect in external-client exclusion: following every same-user `/proc/<pid>/fd` target with `stat` made restore fail closed on an unrelated protected runner descriptor. Linux inspection now reads descriptor links and compares only absolute managed-vault paths, preserving exact live-holder rejection without traversing unrelated targets. Focused recovery checks passed in a Node 24 glibc Linux container while an unrelated protected descriptor remained open.
