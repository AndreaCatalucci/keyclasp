...
The authorized shipping task copied the exact tarball and transcript to `/Users/andreacatalucci/.local/share/keyclasp/releases/0.2.0-beta.1/`, preserved mode `0600`, and reverified both SHA-256 values. It also changed the CI repack into an rc6-equality gate whose expected hash flows to every downstream exact-artifact job. Two workflow attempts stopped before repacking on host-dependent macOS checks. A third passed all source jobs, then proved that Linux x64 with Node 24.19.0/npm 11.17.0 does not reproduce the rc6 archive even though all 167 unpacked file contents and modes match. The final reproducibility job uses macOS arm64 with Node 26.7.0/npm 11.19.0, matching the qualification-host pack toolchain. Committed-source provenance, corrected CI execution, GitHub release creation, npm publication, and registry verification remain in progress.
