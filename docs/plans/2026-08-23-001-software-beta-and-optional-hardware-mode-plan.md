...
The authorized shipping task copied the exact tarball and transcript to `/Users/andreacatalucci/.local/share/keyclasp/releases/0.2.0-beta.1/`, preserved mode `0600`, and reverified both SHA-256 values. It also changed the CI repack into an rc6-equality gate whose expected hash flows to every downstream exact-artifact job. Two pre-release workflow attempts stopped before repacking on host-dependent macOS checks. The final source job compiles and signs the helper source, verifies the reviewed bundle and inventory, skips only the qualification-host byte-equality assertion, and allows 30 seconds for hosted-runner setup hooks. Committed-source provenance, corrected CI execution, GitHub release creation, npm publication, and registry verification remain in progress.
