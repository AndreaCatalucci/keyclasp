...
**Status:** qualification completed on 2026-08-24 against exact candidate rc6. Source tests, the complete supported exact-artifact matrix, unsupported musl rejection, independent review, and the physical Keyclasp Touch ID receipt pass. Durable staging and committed-source provenance are recorded. The CI archive hash differed even though every package path, mode, size, and file hash matched, so CI now verifies a canonical rc6 content manifest and retains its own repack hash for downstream exact-artifact tests. Tagging and publication remain blocked until that corrected CI passes. Slice 5 has not started.
...
- Add software-beta CI that separates source tests from exact-tarball tests on every supported OS and Node version. Treat any CI repack as a reproducibility check: require its canonical path, mode, size, and file-content manifest to equal the frozen candidate, pass the CI archive's own SHA-256 into every downstream job, and keep the frozen candidate as the only publishable artifact. Compression-container byte equality is not a source-provenance requirement. A platform skip is acceptable only when the platform is explicitly unsupported and the package fails closed there.
...
Evidence: 29 source test files pass with 467 tests and 2 deliberate platform skips. macOS arm64 and glibc Linux arm64/x64 pass Node 24 and 26 with reviewed prebuilt and forced source paths. Musl Node 24/26 fails closed before vault creation. The exact rc6 physical run passed all ten interactions; evidence directory `/var/folders/8c/t2tmqjw11gx8pkf90z3p3rbc0000gn/T/keyclasp-slice4-touch-id-GHTyPO`, transcript mode `0600`, SHA-256 `3e99bab30700ef7f12c3e7a2085476ca0a7678048f2235a5b0ec340394eb0d86`. Prior CI source jobs pass; corrected content-manifest CI remains pending. Windows remains unsupported without host evidence.

The authorized shipping task copied the exact tarball and transcript to `/Users/andreacatalucci/.local/share/keyclasp/releases/0.2.0-beta.1/`, preserved mode `0600`, and reverified both SHA-256 values. Final release-source commit C1 is `bded78ad974ddb78f64f53ef40e9ec4f5a6a93e7`; metadata-only C2 is `a513be4462afb9efdb48f87624a7ff3db130e0ce`; the source-tree manifest SHA-256 is `126c53acfacedbf4f5ad8c5099a26485dd4d6d39110e15590dc98c886616d5c5`. Workflow run `32743786645` passed every source job, then produced archive SHA-256 `cf778b637f2b35a1a89ad0a682eee78039b081de7bad96f3dca3d80f234b2471` instead of rc6 SHA-256 `3cef4ddc6c21175e786c9b1bac95d649d1bf3881845d08804914bcff0999ee79`. All 167 unpacked paths, modes, sizes, and file hashes match. The corrected gate records those properties in `docs/releases/0.2.0-beta.1-package-manifest.json`; the exact rc6 archive remains the sole publishable file. No tag, GitHub release, npm publication, registry install, or registry smoke test occurred.
