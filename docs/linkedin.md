...
I built **Keyclasp** to solve this.
...
**How it works:**
- `keyclasp sandbox` replaces every real `.env` value with a deterministic fake
- AI agents see only fakes
- `keyclasp run -- npm start` injects real secrets as env vars for that command only
- `keyclasp unsandbox` restores everything when you're done
