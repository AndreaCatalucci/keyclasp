...
// ─── License Key Generation (same logic as scripts/generate-license.ts) ───
...
function generateLicenseKey(tier: string, email: string, expDate: string): string {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      ...
      return Response.json({
        success: true,
        message: `License key sent to ${customerEmail}`,
      });
    } catch (err: any) {
